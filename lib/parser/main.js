...
const LIST_PROPERTIES = ["id", "file", "classes", "category", "width", "height"];
...
function refresh() {
  options = _.mapKeys(settings.setSettings.get("options"), "id");
  Q
      .all(_.map(setsCtrl.getActiveSets(), function (instance) {
        return instance.use(options[instance.id] || {}).then(_.constant(instance));
      }))
      .done(updateLists);
}
...
function addInstance(instance) {
  if (!options.hasOwnProperty(instance.id) || options[instance.id].mapping) {
    let map = mapping.generate(instance.mapping); // TODO ignore mappings with excluded key
