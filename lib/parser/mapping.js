...
function generate(mapping) {
  ...
  _.each(mapping.separationLeading, function (entry, key) { leadingRE = addLeading(leading, key, entry, leadingRE); });
  _.each(mapping.separationWrapped, function (entry, key) { addWrapped(wrapped, key, entry); });
